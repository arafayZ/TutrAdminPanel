...
const Chat = () => {
  ...
  // Holds a contact object that hasn't been added to the list yet
  ...
  // Guard so the incoming-state effect only runs once per navigation
  const hasProcessedIncoming = useRef(false);

  // Active contact = existing contact OR the pending one
  const activeContact =
    contacts.find((c) => c.id === activeContactId) || pendingContact;

  const activeMessages = activeContactId
    ? messagesMap[activeContactId] || []
    : [];

  // ---------- Handle incoming navigation from Team page (RUN ONCE) ----------
  useEffect(() => {
    const incoming = location.state;
    if (!incoming?.contactName) return;
    if (hasProcessedIncoming.current) return;

    hasProcessedIncoming.current = true;

    const { contactName, contactRole } = incoming;

    // If contact already exists in the sidebar list → just open it
    ...
    if (existing) {
      ...
      // Otherwise: create a *temporary* contact, DO NOT add to sidebar list yet
      ...
      const tempContact = {
        ...
        avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(
          contactName,
        )}&background=1F1F1F&color=fff&size=150`,
        online: true,
      ...
      setPendingContact(tempContact);
      setActiveContactId(tempId);
      setMessagesMap((prev) => ({ ...prev, [tempId]: [] }));
      setShowMobileChat(true);
    }

    // Clear the router state so refresh / back doesn't re-trigger it
  ...
  const handleSendMessage = (e) => {
    ...
    // If we're chatting with a pending (not-yet-in-list) contact,
    // add them to the sidebar list now that a message has been sent.
    if (pendingContact && pendingContact.id === activeContactId) {
      const finalContact = {
        ...pendingContact,
        id: Date.now(), // give them a permanent numeric id
        lastMessage: messageText || "Sent an image",
        time: "Just now",
      };

      // Move the messages over to the new permanent id
      setMessagesMap((prev) => {
        const oldMsgs = prev[pendingContact.id] || [];
        const newMap = { ...prev, [finalContact.id]: oldMsgs };
        delete newMap[pendingContact.id];
        return newMap;
      });

      setContacts((prev) => [finalContact, ...prev]);
      setActiveContactId(finalContact.id);
      setPendingContact(null);
    } else {
      // Normal case: update the last message preview on the sidebar item
      setContacts((prev) =>
        prev.map((c) =>
  ...
  return (
    <div className="flex h-screen bg-slate-50/60 font-sans text-slate-800 overflow-hidden selection:bg-slate-900 selection:text-white">
      ...
      <main className="flex-1 flex overflow-hidden w-full p-0 md:p-4 gap-4">
        {/* CONTACTS SIDEBAR */}
        <div
          className={`w-full md:w-80 bg-white md:rounded-3xl border border-slate-200/80 shadow-xs flex flex-col h-full shrink-0 transition-all duration-300 ${
                ...
                >
                  <div className="relative shrink-0">
                    <img
                      src={contact.avatar}
                      alt={contact.name}
                      className={`w-11 h-11 rounded-full object-cover transition-transform duration-200 ${
        ...
        {/* ACTIVE CHAT SCREEN */}
        <div
          className={`flex-1 flex flex-col h-full bg-white md:rounded-3xl border border-slate-200/80 shadow-xs overflow-hidden w-full ${
            !showMobileChat ? "hidden md:flex" : "flex"
          }`}
        >
                      ...
                      >
                        {msg.image && (
                          <img
                            src={msg.image}
                            alt="Attachment"
                            className="w-full max-h-56 object-cover rounded-xl mb-2.5 border border-slate-100 shadow-xs"
                          />
                        )}
                        {msg.text && (
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 mt-0.5">
                      Upload high-res photos or assets
                    </p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 mt-0.5">
                      Snap directly using your camera
                    </p>
                  </div>
                </button>
              </div>
